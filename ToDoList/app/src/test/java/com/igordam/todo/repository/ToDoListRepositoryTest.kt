...
class ToDoListRepositoryTest {
    ...
    private val repository: ToDoListRepositoryImpl = ToDoListRepositoryImpl(dataSource)
    ...
    fun `getLists - when returns success`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val result = runCatching { repository.getLists() }
            coVerify(exactly = 1) { dataSource.getLists() }
    ...
    fun `getListById - when returns success`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val result = runCatching { repository.getListById(id = 1) }
            coVerify(exactly = 1) { dataSource.getListById(id = 1) }
            assert(result.isSuccess)
        }
    }
    ...
    fun `postLists - when returns success`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val result = runCatching { repository.postLists(mock.postToDoRequest) }
            coVerify(exactly = 1) { dataSource.postLists(mock.postToDoRequest) }
            assert(result.isSuccess)
        }
    }
    ...
    fun `putLists - when returns success`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val result = runCatching { repository.putLists(id = 1, mock.postToDoRequest) }
            coVerify(exactly = 1) { dataSource.putLists(id = 1, mock.postToDoRequest) }
            assert(result.isSuccess)
        }
    }
    ...
    fun `deleteLists - when returns success`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val result = runCatching { repository.deleteLists(id = 1) }
            coVerify(exactly = 1) { dataSource.deleteLists(id = 1) }
            assert(result.isSuccess)
        }
    }
